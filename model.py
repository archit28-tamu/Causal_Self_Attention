...
class CSABlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.to_keys = nn.Linear(config.n_embd, config.n_embd)
        self.to_queries = nn.Linear(config.n_embd, config.n_embd)
        self.to_values = nn.Linear(config.n_embd, config.n_embd)
    ...
    def forward(self, x, layer_past=None):
        ...
        keys = self.to_keys(x).view(B, L, h, c_q).transpose(1, 2) 
        queries = self.to_queries(x).view(B, L, h, c_q).transpose(1, 2)
        values = self.to_values(x).view(B, L, h, c_q).transpose(1, 2)
        ...
        att = att.masked_fill(self.mask[:, :, :L, :L], float('-inf'))
        attn_save = F.softmax(att * c_q**-0.5, dim=-1)
        att = self.attn_drop(attn_save)
        ...
        out = att @ values
        y = out.transpose(1, 2).contiguous().view(B, L, C)
